...
export default function Overview() {
  ...
  const header = el('header.dash-navbar', {}, [
    ...
    el('div', { style: { display: 'flex', gap: '10px', alignItems: 'center' } }, [
      ...
      el('button.btn-ghost', { on: { click: () => go('/analytics') } }, [icons.analytics({ size: 14 }), el('span', { text: 'Отчёт' })]),
      el('button.btn-primary', { on: { click: () => go('/queue') } }, [icons.queue({ size: 14 }), el('span', { text: 'Живая очередь' })]),
    ]),
  ]);
